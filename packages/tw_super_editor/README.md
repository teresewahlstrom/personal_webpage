...
`tw_super_editor` was initiated by [Superlist](https://superlist.com) and is being implemented and maintained by the [Flutter Bounty Hunters](https://flutterbountyhunters.com), Superlist, and the contributors.
...
version of the `SuperEditor` widget by using lower level tools within the `tw_super_editor` package.
