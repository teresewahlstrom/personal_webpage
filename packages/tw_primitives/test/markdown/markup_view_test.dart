...
void main() {
  ...
  testWidgets('horizontal rules render as thin dividers', (tester) async {
    ...
    final dividerRect = tester.getRect(
      find.byWidgetPredicate(
        (Widget widget) => widget is ColoredBox && widget.color == Colors.black,
      ),
    );
    final afterRect = tester.getRect(_richTextWithPlainText('After').first);

    final expectedPerSideSpacing =
        (baseStyle.fontSize ?? 12.0) * (0.75 + 1.2) * (2 / 3);
    expect(dividerRect.height, closeTo(0.4, 0.001));
    expect(
      dividerRect.top - beforeRect.bottom,
      closeTo(expectedPerSideSpacing, 0.5),
    );
    expect(
      afterRect.top - dividerRect.bottom,
      closeTo(expectedPerSideSpacing, 0.5),
    );
  });
  ...
  testWidgets('link pill text does not keep underline decoration', (
    ...
    expect(tester.widget<Text>(find.text('Twin')).style?.fontSize, 13);
  ...
  testWidgets('default link pill text style is independent of ambient text', (
    tester,
  ) async {
    final surface = buildMarkdownSurfaceStyle(
      const MarkdownThemeConfig(isDark: false),
    );
    ...
    final TextStyle? renderedStyle = tester
        .widget<Text>(find.text('Twin'))
        .style;
    final TextStyle expectedStyle = surface.theme.linkPillStyle!.textStyle;

    expect(renderedStyle?.fontSize, expectedStyle.fontSize);
    expect(renderedStyle?.height, expectedStyle.height);
  });
...
MarkupTheme _theme(TextStyle baseStyle, {TwLinkPillStyle? linkPillStyle}) {
  return MarkupTheme(
    ...
    transparentSelectionSpacer: const TextStyle(
      color: Colors.transparent,
      fontSize: 0.01,
      height: 1.0,
    ),
