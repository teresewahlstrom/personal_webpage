...
const EdgeInsets _kTwLinkPillDefaultPadding = EdgeInsets.symmetric(
  horizontal: 6,
  vertical: 2,
);
const double _kTwLinkPillDefaultBorderWidth = 1.0;
const double _kTwLinkPillFillLerpTLight = 0.70;
const double _kTwLinkPillFillLerpTDark = 0.65;
...
class TwLinkPillStyle {
  const TwLinkPillStyle({
    ...
    this.borderWidth = _kTwLinkPillDefaultBorderWidth,
    this.padding = _kTwLinkPillDefaultPadding,
...
TwLinkPillStyle computeDefaultTwLinkPillStyle({
  ...
  final double lerpT = isDark
      ? _kTwLinkPillFillLerpTDark
      : _kTwLinkPillFillLerpTLight;
  final Color fill =
  ...
  final TwTextStyles textStyles = TwTextStyles.forBrightness(brightness);
  final double resolvedScale = (textScale.isFinite && textScale > 0)
      ? textScale.clamp(1.0, 1.6)
      : 1.0;
  final TextStyle pillBase = textStyles.bodyForContextless(
    color: colors.pageBodyText,
    textScale: 1.0,
  );
  final TextStyle pillSmall = textStyles.smallFrom(pillBase);
  final double pillFontSize =
      (pillSmall.fontSize ?? 14.0) * (1 + (resolvedScale - 1) * 0.7);
  final double pillHeight =
      (pillSmall.height ?? 1.0) * (1 + (resolvedScale - 1) * 0.18);
  final TextStyle textStyle = textStyles.adaptBase(
    pillSmall,
  ...
  return TwLinkPillStyle(
    fillColor: fill,
    borderColor: border,
    borderWidth: _kTwLinkPillDefaultBorderWidth,
    padding: _kTwLinkPillDefaultPadding,
    textStyle: textStyle,
    shadows: shadows,
  );
}
...
class TwLinkPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final TwLinkPillStyle resolved = style ?? _resolveDefault(context);
    ...
    Widget content;
    if (isClickable) {
      final Widget inkContent = label.isEmpty
