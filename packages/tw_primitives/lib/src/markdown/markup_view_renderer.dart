...
class MarkupViewRenderer {
  ...
  _MarkupLayoutNode _buildLayoutBlock(
    ...
    switch (block) {
      ...
      case MarkupHorizontalRuleBlock():
        final Color ruleColor = chromeVisible
            ? (theme.baseStyle.color ??
                  DefaultTextStyle.of(context).style.color ??
                  Colors.transparent)
            : context.twColors.transparent;
        return _MarkupLayoutHorizontalRuleNode(
          color: ruleColor,
          height: _style.blockquoteRailWidth,
        );
      case final MarkupBlockquoteBlock quote:
        ...
        return _MarkupLayoutBlockquoteNode(
          ...
          contentPadding: EdgeInsets.fromLTRB(
            20,
            MarkupViewStyle.blockquoteRailVerticalOverhang +
                MarkupViewStyle.blockquoteInnerVerticalPadding,
            0,
            MarkupViewStyle.blockquoteRailVerticalOverhang +
                MarkupViewStyle.blockquoteInnerVerticalPadding,
          ),
  ...
  Widget _renderLayoutNode(_MarkupLayoutNode node) {
    switch (node) {
      ...
      case final _MarkupLayoutHorizontalRuleNode rule:
        return SizedBox(
          width: double.infinity,
          height: rule.height,
          child: ColoredBox(color: rule.color),
        );
  ...
  double _blockSpacing(
    ...
    final double fontSize =
        theme.baseStyle.fontSize ??
        context.twTextStyleTokens.twBodyBaseFontSize;
    ...
    if (previousBlock is MarkupHorizontalRuleBlock ||
        nextBlock is MarkupHorizontalRuleBlock) {
      spacing += fontSize * _style.horizontalRuleExtraSpacing;
      spacing *= _style.horizontalRuleSpacingScale;
    }
    if (!inListItem &&
  ...
  double _listItemSpacing(TextStyle baseStyle, {required int listDepth}) {
    final double fontSize =
        baseStyle.fontSize ?? context.twTextStyleTokens.twBodyBaseFontSize;
...
class _MarkupLayoutHorizontalRuleNode extends _MarkupLayoutNode {
  const _MarkupLayoutHorizontalRuleNode({
    required this.color,
