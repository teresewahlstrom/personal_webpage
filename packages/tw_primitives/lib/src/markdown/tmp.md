...
- High-level surface (use `buildMarkdownSurfaceStyle`):
  ...
  - `MarkdownThemeConfig.linkPillStyle` — optional `TwLinkPillStyle`. When omitted a sensible default is provided by `tw_primitives` so callers don't need to construct one.
...
- Theme & pill customization:
  ...
  - `TwLinkPillStyle` fields you can set/copyWith: `fillColor`, `borderColor`, `borderWidth`, `padding`, `shadows`, `textStyle`.
...
- **Link pill visuals:** `MarkdownThemeConfig.linkPillStyle` / `TwLinkPillStyle` (fillColor, borderColor, textStyle, shadows). See landing_page.dart and message_bubble.dart.  
