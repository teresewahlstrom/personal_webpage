...
class IOSTextFieldTouchInteractorState extends State<IOSTextFieldTouchInteractor>
  ...
  void didChangeMetrics() {
    if (widget.focusNode.hasFocus) {
      _ensureExtentVisibleAfterLayout();
    }
  }
  ...
  void _ensureExtentVisibleAfterLayout() {
    onNextFrame((_) {
      if (!mounted || !widget.focusNode.hasFocus || widget.textController.selection.extentOffset < 0) {
        return;
      }

      widget.textScrollController.ensureExtentIsVisible();

      // Keyboard insets can finalize on a subsequent frame.
      onNextFrame((_) {
        if (!mounted || !widget.focusNode.hasFocus || widget.textController.selection.extentOffset < 0) {
          return;
        }
