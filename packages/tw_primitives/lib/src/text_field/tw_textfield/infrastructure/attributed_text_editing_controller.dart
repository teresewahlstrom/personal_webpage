...
import 'package:tw_primitives/src/text_field/infrastructure/strings.dart';
...
class AttributedTextEditingController with ChangeNotifier {
  ...
  void _moveCaretUpstream({
    ...
    if (movementModifier == MovementModifier.line) {
      final newExtent = textLayout.getPositionAtStartOfLine(selection.extent);
      selection = TextSelection(
        baseOffset: expandSelection ? selection.baseOffset : newExtent.offset,
  ...
  void _moveCaretDownstream({
    ...
    if (movementModifier == MovementModifier.line) {
      final newExtent = textLayout.getPositionAtEndOfLine(selection.extent);
