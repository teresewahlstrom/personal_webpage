...
**3. Method naming inconsistency: `socialFor/socialHoverFor`** 
- File: app_ui_config.dart
- Current: `PagePalette.socialFor()`
- Issue: Semantically it resolves `pageAccent` color. "Social" is task-specific; generalize for reuse
- **Recommend**: Rename to `accentFor()` and `accentHoverFor()` (or `primaryFor()` / `primaryHoverFor()`)

**4. Line theme roles inconsistent casing**
- File: app_line_theme.dart
- Current: `subtleFor`, `subtle2For`, `subtle3For`, `accent1For` 
- Issue: `accent1` has numeric suffix but others don't; awkward naming for numbered variants
- **Recommend**: Use semantic names like `subtleFor()`, `subtleSecondaryFor()`, `accentFor()` or rename to `lineSubtle`, `lineSubtleAlt`, `lineAccent`
